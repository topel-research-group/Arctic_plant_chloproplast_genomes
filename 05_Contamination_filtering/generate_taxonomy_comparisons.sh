...
for TAX in `find ./* -maxdepth 0 -type d`; do
    ...
    for i in {1..5}; do
        ...
        (while read ptn; do grep $ptn ../Latnja_chloroplast_sample_full_taxonomy_updated.tsv; done < column_untranslated.txt) | cut -d$'\t' -f2 > "column_${i}.txt"
    done
    ...
    #Exract Family + genus for both full and duplicate only datasets
    perl -pe  's/\s\w+;\w+;\w+;\w+;//g' < $FILEOUT | perl -pe  's/;[\w-\.?]+;[\w-\.?]+[ \t]//g' | perl -pe  's/;[\w-\.?]+;+[ \t]//g' | perl -pe  's/;[\w-\.;?]+$//g' > "${GENE}_BLAST_top_ncbi_hit_and_self_top_4_out_family.txt"
    perl -pe  's/\s\w+;\w+;\w+;\w+;//g' < $FILEOUT2 | perl -pe  's/;[\w-\.?]+;[\w-\.?]+[ \t]//g' | perl -pe  's/;[\w-\.?]+;+[ \t]//g' | perl -pe  's/;[\w-\.;?]+$//g' > "${GENE}_BLAST_top_ncbi_hit_and_self_top_4_out_family_duplicates_only.txt"
...
#generate summaries containing all hybrid contigs and mismatches between contig matched, and intended taxon respectively
sort -k9 -n -t$'\t' */*_BLAST_top_ncbi_hit_and_self_top_4_out_family_contig_summary.txt | cut -d$'\t' -f1,2,3,8,9,10,11 | sed 's/_00...//g' | sed 's/%//g' | awk -F'\t' '$5<70' | sort -k1 | uniq > potential_hybrid_contigs.txt
sort -k9 -n -t$'\t' */*_BLAST_top_ncbi_hit_and_self_top_4_out_family_contig_summary.txt | cut -d$'\t' -f1,2,3,8,9,10,11 | sed 's/_00...//g' | sed 's/%//g' | awk -F'\t' '$5>=70' | sort -k1 | uniq | awk '$2!=$4' > mismatched_contigs.txt
